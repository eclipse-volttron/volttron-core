...
from volttron.utils.commands import (is_volttron_running,
...
AgentMeta = collections.namedtuple("Agent",
...
def _list_agents(opts) -> List[AgentMeta]:
    ...
    return [
        AgentMeta(a["name"], a.get("tag", ""), a["uuid"], a["identity"],
                  a.get("agent_user"),
                  ) for a in agents
    ]
...
def _show_filtered_agents_status(opts,
    ...
    if not opts.json:
        ...
        if cc.is_secure_mode():
            ...
            fmt = "{} {:{}} {:{}} {:{}} {:{}} {:>6} {:>15}\n"
            _stderr.write(
                fmt.format("UUID", "AGENT", name_width, "IDENTITY",
