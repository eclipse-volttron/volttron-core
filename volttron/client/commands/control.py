...
AgentMeta = collections.namedtuple(
    "Agent", "name tag uuid vip_identity agent_user"
)
...
def backup_agent_data(output_filename, source_dir):
    with tarfile.open(output_filename, "w:gz") as tar:
        tar.add(
            source_dir, arcname=os.path.sep
    ...
    for pattern, match in filter_agents(agents, opts.pattern, opts):
        ...
        for agent in match:
            ...
            opts.connection.call(
                "remove_agent", agent.uuid, remove_auth=remove_auth
            )
...
def list_remotes(opts):
    ...
    try:
        pending_certs = conn.server.vip.rpc.call(
            AUTH, "get_authorization_pending"
        ).get(timeout=4)
...
def approve_remote(opts):
    ...
    conn.server.vip.rpc.call(
        AUTH, "approve_authorization_failure", opts.user_id
    ).get(timeout=4)
...
def deny_remote(opts):
    ...
    if not conn:
        ...
        return
    conn.server.vip.rpc.call(
        AUTH, "deny_authorization_failure", opts.user_id
    ).get(timeout=4)
...
def delete_remote(opts):
    ...
    if not conn:
        ...
        return
    conn.server.vip.rpc.call(
        AUTH, "delete_authorization_failure", opts.user_id
    ).get(timeout=4)
...
def _print_two_columns(dict_, key_name, value_name):
    ...
    _stdout.write(
        "{}{}{}\n".format(
            key_name, " " * (max_key_len - len(key_name)), value_name
        )
    )
    _stdout.write(
        "{}{}{}\n".format(
...
def _ask_for_auth_fields(
    ...
    **kwargs,
    ...
    asker.add(
        "roles", roles, "delimit multiple entries with comma", _comma_split
    )
    asker.add(
        "groups", groups, "delimit multiple entries with comma", _comma_split
    )
...
def _show_filtered_agents_status(
    opts, status_callback, health_callback, agents=None
):
    """Provides generic way to filter and display agent information.
...
def main():
    ...
    global_args = config.ArgumentParser(
        description="global options", add_help=False
    )
    ...
    tag = add_parser(
        "tag", parents=[filterable], help="set, show, or remove agent tag"
    )
    ...
    group.add_argument(
        "-r", "--remove", action="store_true", help="remove tag"
    )
    ...
    peers = add_parser(
        "peerlist", help="list the peers connected to the platform"
    )
    peers.set_defaults(func=list_peers)

    list_ = add_parser(
        "list", parents=[filterable], help="list installed agent"
    )
    ...
    status = add_parser(
        "status", parents=[filterable], help="show status of agents"
    )
    ...
    start = add_parser(
        "start", parents=[filterable], help="start installed agent"
    )
    ...
    auth_publickey.add_argument(
        "pattern", nargs="*", help="UUID or name of agent"
    )
    auth_publickey.add_argument(
        "-n",
        dest="min_uuid_len",
    ...
    auth_update.add_argument(
        "index", type=int, help="index of record to update"
    )
    ...
    auth_update_group.add_argument(
        "group", metavar="GROUP", help="name of group"
    )
    auth_update_group.add_argument(
        "roles",
        nargs="*",
    ...
    # config commands
    # ====================================================
    config_store = add_parser(
        "config", help="manage the platform configuration store"
    )
    ...
    config_store_store.add_argument(
        "identity", help="VIP IDENTITY of the store"
    )
    config_store_store.add_argument(
    ...
    config_store_store.set_defaults(
        func=add_config_to_store, config_type="json"
    )
    ...
    stats = add_parser(
        "stats", help="manage router message statistics tracking"
    )
