...
def cron(cron_string, start=None, stop=None, second=0):
    ...
    def _weekdays(year, month, day=1):
        ...
        dt += timedelta(
            weekdays[i] - weekday if i < len(weekdays) else weekdays[0] + 7 - weekday
        )
        ...
        if (
            ...
            if start.hour in hours:
                for minute in minutes[bisect_right(minutes, start.minute) :]:
                    yield datetime(
                        start.year, start.month, start.day, start.hour, minute, second
                    )
            for hour in hours[bisect_right(hours, start.hour) :]:
                for minute in minutes:
                    yield datetime(
                        start.year, start.month, start.day, hour, minute, second
                    )
