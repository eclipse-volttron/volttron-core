...
def list_remotes(opts):
    ...
    try:
        approved_certs = conn.server.vip.rpc.call(AUTH,
                                                  "get_approved_authorizations").get(timeout=4)
...
def add_auth_parser(add_parser_fn, filterable):
    auth_cmds = add_parser_fn("auth", help="manage authorization entries and encryption keys")

    auth_subparsers = auth_cmds.add_subparsers(title="subcommands",
                                               metavar="",
                                               dest="store_commands")

    auth_add = add_parser_fn("add",
